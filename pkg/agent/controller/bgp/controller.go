...
func (c *Controller) syncBGPPolicy(ctx context.Context) error {
	...
	if needUpdateBGPServer {
		...
		if ok, err := validatePolicy(effectivePolicy); !ok {
			klog.V(2).InfoS("Invalid BGPPolicy", "Error", err, "BGPPolicy", klog.KObj(effectivePolicy))
			return nil
		}
...
func generateBGPPeerKey(address string, asn int64) string {
...
func (c *Controller) GetBGPPolicyInfo() (string, string, int64, int32) {
...
func validatePolicy(policy *v1alpha2.BGPPolicy) (bool, error) {
	if policy.Spec.LocalASN == 0 {
		return false, errors.New("LocalASN was 0")
	}

	for _, bgpPeer := range policy.Spec.BGPPeers {
		if bgpPeer.ASN == 0 {
			return false, errors.New("Peer ASN was 0")
		}
	}

	if policy.Spec.Confederation != nil {
		if policy.Spec.Confederation.Identifier == 0 {
			return false, errors.New("Confederation identifier was 0")
		}

		for _, memberAsn := range policy.Spec.Confederation.MemberASNs {
			if memberAsn == 0 {
				return false, errors.New("Confederation has member with ASN 0")
			}
		}
	}
